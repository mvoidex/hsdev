	...
	sub l r = Database {
		...
		where
			diff' x y = if M.null z then Nothing else Just z where
				z = M.difference x y
	zero = Database M.empty M.empty M.empty M.empty
...
fromModule m = fromMaybe (error "Module must specify source file or cabal") $ inSource `mplus` inCabal where
	inSource = do
		...
		return $ createIndexes $ Database mempty (M.singleton (locationFile loc) m) zero zero
	inCabal = do
		cabal <- moduleCabal $ symbol m
		return $ createIndexes $ Database (M.singleton cabal (M.singleton (symbolName m) m)) mempty zero zero
