...
goToDeclaration db file qualifier ident = do
	...
	let
		modules = filter bySources $ mapMaybe symbolModule $ S.toList decls
		candidates = maybe (filter bySources $ S.toList decls) findCandidates $ M.lookup file' (databaseFiles db)
...
completions db file qualifier prefix = do
	file' <- liftIO $ canonicalizePath file
	return (result file')
	where
